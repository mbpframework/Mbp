...
<script>
import { AddTrainPlanYear, UpdateTrainPlanYear, GetTrainPlanYears, DeleteTrainPlanYear } from '@/api/bll/plan/yearplan'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      uploadData: { AttachmentTypeElementCode: 'document', BussinessTypeElementCode: 'TrainPlanYear', BussinessId: guid() },
      attachList: [],
  ...
  methods: {
    handleBeforeUpload(file) {
      ...
      this.uploadData.BussinessTypeElementCode = 'TrainPlanYear'
    ...
    getList() {
      this.listLoading = true
      GetTrainPlanYears(this.listQuery).then(response => {
    ...
    createData() {
      ...
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          AddTrainPlanYear(this.temp).then(() => {
    ...
    updateData() {
      ...
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          ...
          UpdateTrainPlanYear(tempData).then(() => {
    ...
    handleDelete(row) {
      DeleteTrainPlanYear(row.Id).then(() => {
        this.$notify({
          title: 'Success',
